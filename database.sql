...
-- select movie genres from movie id
SELECT genres.name FROM movies 
JOIN movies_genres ON movies.id = movies_genres.movie_id
JOIN genres ON movies_genres.genre_id = genres.id
WHERE movies.id = 1;

-- update movies based on id
UPDATE movies 
SET title = 'test', description = 'test', poster='images/avatar.jpeg'
WHERE id = 16;

-- delete movies and genres based on id
DELETE FROM movies_genres WHERE movie_id = 16;
DELETE FROM movies WHERE id = 16;
